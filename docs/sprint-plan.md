...
| TC-016 기본 이동/점프 플레이 흐름 | 기본 플레이 흐름 | 실제 플레이 화면에서 짧은 이동/점프 조합 수행 |
...
| TC | 플레이어 상태 | 목적 |
| --- | --- | --- |
| TC-013 최초 실행 상태 식별 | PLAYER-NEW | 세이브 데이터가 없는 상태에서 첫 진입 흐름 확인 |
| TC-014 기존 플레이 상태 식별 | PLAYER-RETURNING | 세이브 데이터가 있는 상태에서 Continue 또는 진행 상태 표시 확인 |
| TC-015 세이브 상태 보존 확인 | PLAYER-RETURNING | 자동화 실행 후 기존 세이브 데이터 손상 여부 확인 |
...
## Sprint 7: Evidence 검증

목표: 실패 또는 판단 불가 상황에서 evidence가 일관되게 저장되는지 확인한다.
...
다음 작업은 TC가 아니라 리포트/문서 개선 작업으로 관리한다.

| 작업 | 목적 |
| --- | --- |
| Markdown 실행 요약 리포트 생성 | JSON evidence를 사람이 읽기 좋은 Markdown 문서로 정리 |
