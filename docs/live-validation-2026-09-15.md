...
이번은 수정 후 정상 경로 1회씩이다. 실제 크기 변경·캡처 오류를 주입해 이벤트 보존을 검증한 것은 아니다. 이전 TC-010 PASS는 이전 코드의 별도 실행이며 최신 코드 3회 반복으로 합산하지 않는다. TC-011 방향키 시각 반응, 동일 시작 조건 반복, 독립 이미지 평가가 남아 있다. 이전 실패·보류 이력은 [9월 14일 기록](live-validation-2026-09-14.md)에 유지한다.
